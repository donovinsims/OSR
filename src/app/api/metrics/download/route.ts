...
import { db } from '@/db';
import { agents, agentMetrics } from '@/db/schema';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Upsert into agent_metrics using INSERT ... ON CONFLICT
    await db.run(sql`
      INSERT INTO agent_metrics (agent_id, date, downloads, visits, shares)
    ...
    console.error('POST /api/metrics/download error:', error);
    return NextResponse.json({ 
